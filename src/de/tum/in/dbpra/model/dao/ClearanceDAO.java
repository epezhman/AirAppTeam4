...
public class ClearanceDAO extends AbstractDAO {
	...
	public boolean giveClearance(String airport_id,
			Integer flight_controller_id, Integer flight_segment_id,
			Date clearance_time) throws SQLException {
		...
		try (Connection connection = getConnection()) {
			...
			PreparedStatement preparedStatementInsertClearance = connection
					.prepareStatement(InsertClearance,
							java.sql.ResultSet.TYPE_SCROLL_INSENSITIVE,
							ResultSet.CLOSE_CURSORS_AT_COMMIT);

			try (ResultSet resultset = preparedStatementNumberOfRunwaysQuery
					.executeQuery()) {
				...
				try (ResultSet resultset2 = preparedStatementRunwayQuery
					...
					for (int i = 1; i <= k; i++) {
						if (!resultset2.next()) {
							freeRunway = i; // Get the smallest id of the free
											// runways.
							break;
						}
					}
					...
					if (freeRunway == 0) {
						return false; // There is no free runway for the
										// selected clearance_time.
					}

					preparedStatementInsertClearance.setInt(1, freeRunway);
					try {
						preparedStatementInsertClearance.executeUpdate(); // Insert
																			// Clearance
																			// into
																			// runway_log.
						return true;
