...
public class FlightSegmentDAO extends AbstractDAO {
	 ...
	 * 
	 * */
	public List<FlightSegmentBean> searchFlight(String to, String from, String toDate,
	...
	private String constructConnFlightQuery(String to, String from,
		...
		.append(" where origin.city ='").append(from).append("'").append(" INTERSECT ")
		...
		.append(" target1.city='").append(to).append("' AND (select departure_time::timestamp::date)  >=")
		.append(" '").append(fromDate).append("' )");

		if(isOneway.equals("false")){
			//query for retrieving common connection bw origin and destination
			query.append(" UNION ( with connection2 as ( select target.city as conn_city from flight_segment fs2 ")
			.append(" inner join airport target on fs2.airport_destination_id = target.airport_id")
			.append(" inner join airport origin on fs2.airport_departure_id = origin.airport_id")
			.append(" where origin.city ='").append(to).append("'").append(" INTERSECT ")
			.append(" select target.city as conn_city from flight_segment fs1")
			.append(" inner join airport target on fs1.airport_destination_id = target.airport_id")
			.append(" inner join airport origin on fs1.airport_departure_id = origin.airport_id")
			...
			.append(" target1.city='").append(from).append("' AND (select departure_time::timestamp::date)  >=")
			.append(" '").append(toDate).append("' )");
			
			orderBy.append("( ").append(query).append(" order by departure_time asc ) ");
			System.out.println("connection flight query with order by "+orderBy);
	...
	private String constructDirectFlightQuery(String to, String from,
		...
		.append(" origin.city ='").append(from).append("' AND target.city='").append(to).append("'")
		...
		if (isOneway.equals("false")){
			...
			.append(" inner join airline ar on fs.airline_id = ar.airline_id")
			.append(" inner join airplane ap on fs.airplane_id = ap.airplane_id where ")
			.append(" origin.city ='").append(to).append("' AND target.city='").append(from).append("'")
			.append("  AND (select departure_time::timestamp::date)  >='").append(toDate).append("' limit 7 )");
		}
	 ...
	 * 
	 * */
	public List<String> searchReturnFlight(String to, String from, String toDate,
			String fromDate, String ticketClass,String noOfPass) throws SQLException {
		...
		try (Connection connection = getConnection();
			...
			try (ResultSet rs = preparedStatement.executeQuery();) {
				while (rs.next()) {
					...
					flightSegmentBean.setDepartureTime(rs.getTimestamp("departure_time"));
					flightSegmentBean.setArrivalTime(rs.getTimestamp("arrival_time"));
