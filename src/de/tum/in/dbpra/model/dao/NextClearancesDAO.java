...
/**
 * List of the free runways plus possible clearance time within clearance_time +
 * 30 mins
 * 
 * @param airport_id
...
public class NextClearancesDAO extends AbstractDAO {
	...
	public List<Object> giveNextClearances(String airport_id,
			Date clearance_time) throws SQLException, ParseException {

		List<Object> list = new ArrayList<Object>();
		String runwayQuery = "Select rl.runway_id "
				+ "From runway_log rl, runway r "
				+ "Where rl.runway_id=r.runway_id and r.airport_id='"
				+ airport_id
				+ "' and rl.runway_id not in "
		...
		try (Connection connection = getConnection()) {
			...
			for (int j = 0; j <= 6; j++) {
				if (j == 0) {
					...
					clearance_time = addMinutesToDate(5, clearance_time);
					// Add 5 minutes to the clearance_time
				}
				long time = clearance_time.getTime();
				Timestamp timestamp = new Timestamp(time);
				preparedStatementRunwayQuery.setTimestamp(1, timestamp);
				SimpleDateFormat sdf = new SimpleDateFormat(
						"yyyy-M-dd hh:mm:ss"); // Right format for
												// clearance_time.

				try (ResultSet resultset2 = preparedStatementRunwayQuery
						.executeQuery()) {

					resultset2.beforeFirst();
					int l = 0;
					while (resultset2.next()) {
						l = resultset2.getInt(1);
						// Runway_id of the free runway.
						m++;
						list.add(2 * m, l); // Add runway_id to the return-list.
						String clearance_time_in_string = sdf
								.format(clearance_time);
						clearance_time = sdf.parse(clearance_time_in_string);
						Timestamp timestamp2 = new java.sql.Timestamp(
								clearance_time.getTime());
						// Convert clearance_time into right format.
						clearance_time = timestamp2;

						list.add(2 * m + 1, clearance_time);
						// Add corresponding clearance_time to the return-list.
					}
